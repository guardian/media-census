...
case class HistogramDataResponse[T:io.circe.Encoder,V:io.circe.Encoder,N:io.circe.Encoder] (status:String, buckets:List[T], values:List[V], extraData:Option[N])

object HistogramDataResponse {
  private val logger = LoggerFactory.getLogger(getClass)

  def apply[T:io.circe.Encoder,V:io.circe.Encoder](status: String, buckets: List[T], values: List[V]): HistogramDataResponse[T,V,Unit] =
    new HistogramDataResponse(status, buckets, values,None)
  ...
  def fromMap[T:io.circe.Encoder,V:io.circe.Encoder,N:io.circe.Encoder](data:Map[T,V], extraData:Option[N]) = {
    new HistogramDataResponse[T,V,N]("ok",data.keys.toList, data.values.toList, extraData)
  }
}
