...
import streamcomponents.DeletableSwitch
import vidispine.VSFile

import scala.concurrent.Await
import scala.concurrent.duration._

class DeletableSwitchSpec extends Specification {
  "DeletableSwitch" should {
    "Push a report indicating true to YES" in new AkkaTestkitSpecs2Support {
      ...
      val testStream = GraphDSL.create(sinkFactory) { implicit builder=> sink=>
        ...
        val src = builder.add(Source.single(UnclogStream(VSFile("VX-123344", "/test", "/test", None, 2318793, None, ZonedDateTime.parse("2019-10-16T04:00:00.523+01:00", pattern), 1, "VX-4", None, None, None, None),None,Seq(VidispineProject("VX-1234", Option("Test Name"), Option("New"), Option("VX-12"), true, true, true, Option(ZonedDateTime.parse("2019-07-17T10:35:12.598Z", pattern)), Option(ZonedDateTime.parse("2019-10-16T04:00:00.523+01:00", pattern)))), None)))
        val switch = builder.add(new DeletableSwitch)
