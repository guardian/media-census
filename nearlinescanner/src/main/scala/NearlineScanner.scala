...
object NearlineScanner {
  ...
  lazy val indexer = new VSFileIndexer(indexName)
  ...
  def buildStream(esClient:ElasticClient, storageId:String) = {
    ...
    GraphDSL.create(counterSink) { implicit builder=> counter=>
      ...
      val src = builder.add(new VSStorageScanSource(storageId, vsConfig.vsUri, vsConfig.plutoUser, vsConfig.plutoPass,pageSize=100))
